...
const PropertyRequestList = require("./propertyrequestlist.js");
// const QueryUtils = require('./queries.js');

/**
 * A custom context provides easy access to list of all properties
 */
class PropertyRequestContext extends Context {
    constructor() {
        super();
...
/**
 * Define PropertyRequest smart contract by extending Fabric Contract class
 *
 */
class PropertyRequestContract extends Contract {
    constructor() {
        // Unique namespace when multiple contracts (PropertyRequest and PropertyRequest-request) per chaincode file
        super(PropertyRequest.getClass());
    }

    /**
     * Define a custom context for PropertyRequest
     */
    createContext() {
        return new PropertyRequestContext();
    }
    ...
    /**
     * Issue PropertyRequest (without any validations)
     * // requestID, propertyID, buyer, amount
     * @param {Context} ctx the transaction context
    ...
    async issue(ctx, PropertyID, RequestID, buyer, amount) {
        ...
        property_request.setInitiated();

        let mspid = ctx.clientIdentity.getMSPID();
        property_request.setBuyerMSP(mspid);
    ...
    async AcceptOffer(ctx, RequestID, buyer) {
        ...
        if (!property_request.isInitiated()) {
            throw new Error("First Get It Initiated!");
        }
        if (property_request.isAcceptedBySeller()) {
            throw new Error("Already Accepted By Seller!");
        }
        if (property_request.isRejected()) {
            throw new Error("Error! This Was Rejected.");
        }

        property_request.setAcceptedBySeller();
    ...
    async FinalizeProperty(ctx, RequestID, buyer) {
        // get PropertyRequest from key
        let propertyRequestKey = PropertyRequest.makeKey([RequestID]);
        let property_request = await ctx.propertyRequestList.getPropertyRequest(
            propertyRequestKey
        ...
        if (property_request.isFinalizedByBuyer()) {
            throw new Error("Already Finalized By Buyer!");
        }
        if (property_request.isRejected()) {
    ...
    async MakePayment(ctx, RequestID, buyer) {
        ...
        if (!property_request.isFinalizedByBuyer()) {
            throw new Error("First Get Finalized By Buyer!");
        }
        if (property_request.isPaymentDone()) {
            throw new Error("Already Payment Done By Buyer!");
        }
        if (property_request.isRejected()) {
            throw new Error("Error! Already Rejected.");
        }
...
module.exports = PropertyRequestContract;
